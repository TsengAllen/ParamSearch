...
for filename in selectFile(STName,STIMPnLFolder):
    ...
    for rd in range(1,4):
        ...
        for i in range(1,6):
            ...
            TTestDic['PnLTTest'] += [GETTtestDF(prodata.PnLStdYBY,prodata.PnLTopYBY,IS,OOS,rd)]
            TTestDic['MDDTTest'] += [GETTtestDF(prodata.MDDStdYBY,prodata.MDDTopYBY,IS,OOS,rd)]
            TTestDic['MARTTest'] += [GETTtestDF(prodata.MARStdYBY,prodata.MARTopYBY,IS,OOS,rd)]
            
            if rd == 1:
                prodata.PnLTop.columns = ['圈數：0'];prodata.MDDTop.columns = ['圈數：0'];prodata.MARTop.columns = ['圈數：0']
                ProfitDic['ChoseByPnL'] += [pd.concat([prodata.PnLTop,prodata.PnLStd],axis = 1)]
                ProfitDic['ChoseByMDD'] += [pd.concat([prodata.MDDTop,prodata.MDDStd],axis = 1)]
                ProfitDic['ChoseByMAR'] += [pd.concat([prodata.MARTop,prodata.MARStd],axis = 1)]
            else:
                ProfitDic['ChoseByPnL'] += [prodata.PnLStd]
                ProfitDic['ChoseByMDD'] += [prodata.MDDStd]
                ProfitDic['ChoseByMAR'] += [prodata.MARStd]
    ...
    for key,values in IndicTTest.items():
        ...
        name = './Result/' + STName + '/'+ STName +'_'+IMName+'/TTestStd/'
        isdirAlive(name)
        name += key + '.csv'
        df.to_csv(name,encoding = 'cp950')
    for key,values in IndicProfit.items():
        df = pd.concat(values,axis = 1)
        name = './Result/' + STName + '/'+ STName +'_'+IMName+'/ProfitStd/'
        isdirAlive(name)
        name += key + '.csv'
        df.to_csv(name,encoding = 'cp950')
