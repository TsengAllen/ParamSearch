...
for filename in selectFile(STName,STIMPnLFolder):
    ...
    for quant in [0.99,0.95,0.9]:
        ...
        for i in range(1,6):
            ...
            d.ApplyRGV(quant)
            prodata = ProfitData(d,quant)
            prodata.ApplyRgv()

            TTestDic['PnLTTest'] += [GETTtestDF(prodata.PnLRgvYBY,prodata.PnLTopYBY,IS,OOS,quant)]
            TTestDic['MDDTTest'] += [GETTtestDF(prodata.MDDRgvYBY,prodata.MDDTopYBY,IS,OOS,quant)]
            TTestDic['MARTTest'] += [GETTtestDF(prodata.MARRgvYBY,prodata.MARTopYBY,IS,OOS,quant)]
            
            if quant == 0.99:
                prodata.PnLTop.columns = ['圈數：0'];prodata.MDDTop.columns = ['圈數：0'];prodata.MARTop.columns = ['圈數：0']
                ProfitDic['ChoseByPnL'] += [pd.concat([prodata.PnLTop,prodata.PnLRgv],axis = 1)]
                ProfitDic['ChoseByMDD'] += [pd.concat([prodata.MDDTop,prodata.MDDRgv],axis = 1)]
                ProfitDic['ChoseByMAR'] += [pd.concat([prodata.MARTop,prodata.MARRgv],axis = 1)]
            else:
                ProfitDic['ChoseByPnL'] += [prodata.PnLRgv]
                ProfitDic['ChoseByMDD'] += [prodata.MDDRgv]
                ProfitDic['ChoseByMAR'] += [prodata.MARRgv]
                
            print(IMName,IS,OOS,quant)
        #分別輸出三個不同指標篩選的TTest
    ...
    for key,values in IndicTTest.items():
        ...
        name = './Result/' + STName + '/'+ STName +'_'+IMName+'/TTestRgv/'
        isdirAlive(name)
        name += key + '.csv'
        df.to_csv(name,encoding = 'cp950')
    for key,values in IndicProfit.items():
        df = pd.concat(values,axis = 1)
        name = './Result/' + STName + '/'+ STName +'_'+IMName+'/ProfitRgv/'
        isdirAlive(name)
        name += key + '.csv'
        df.to_csv(name,encoding = 'cp950')