...
for filename in selectFile(STName,STIMPnLFolder):
    ...
    for quant in [0.99,0.95,0.9]:
        ...
        for i in range(1,6):
            ...
            TTestDic['PnLTTest'] += [GETTtestDF(prodata.PnLRgaYBY,prodata.PnLTopYBY,IS,OOS,quant)]
            TTestDic['MDDTTest'] += [GETTtestDF(prodata.MDDRgaYBY,prodata.MDDTopYBY,IS,OOS,quant)]
            TTestDic['MARTTest'] += [GETTtestDF(prodata.MARRgaYBY,prodata.MARTopYBY,IS,OOS,quant)]
            
            if quant == 0.99:
                prodata.PnLTop.columns = ['圈數：0'];prodata.MDDTop.columns = ['圈數：0'];prodata.MARTop.columns = ['圈數：0']
                ProfitDic['ChoseByPnL'] += [pd.concat([prodata.PnLTop,prodata.PnLRga],axis = 1)]
                ProfitDic['ChoseByMDD'] += [pd.concat([prodata.MDDTop,prodata.MDDRga],axis = 1)]
                ProfitDic['ChoseByMAR'] += [pd.concat([prodata.MARTop,prodata.MARRga],axis = 1)]
            else:
                ProfitDic['ChoseByPnL'] += [prodata.PnLRga]
                ProfitDic['ChoseByMDD'] += [prodata.MDDRga]
                ProfitDic['ChoseByMAR'] += [prodata.MARRga]
    ...
    for key,values in IndicTTest.items():
        ...
        name = './Result/' + STName + '/'+ STName +'_'+IMName+'/TTestRga/'
        isdirAlive(name)
        name += key + '.csv'
        df.to_csv(name,encoding = 'cp950')
    for key,values in IndicProfit.items():
        df = pd.concat(values,axis = 1)
        name = './Result/' + STName + '/'+ STName +'_'+IMName+'/ProfitRga/'
        isdirAlive(name)
        name += key + '.csv'
        df.to_csv(name,encoding = 'cp950')